...
// ===============================================================
__global__ void solveLaplaceNFEqCoreGPU(cufftDoubleComplex* phi_complex, double* phi, double* f, double* k2n, int Nx, int Ny, int Nbx, int Nby, int getType) {
    ...
    if (j<Nx && i<Ny) {
        if (getType==0) {
            ...
            if (k2n[idx] == 0){
                ...
                phi_complex[idx].x = phi_complex[idx].x/(k2n[idx]);
                phi_complex[idx].y = phi_complex[idx].y/(k2n[idx]);
            };
        } else if (getType==2) {
